...
class RalphOrchestrator:
    ...
    async def _aexecute_iteration(self) -> bool:
        ...
        if isinstance(self.current_adapter, OllamaAdapter):
            adapter_kwargs["model"] = self.ollama_model
        ...
        if not response.success and not self.allow_fallbacks:
            error_msg = response.error or "Unknown error"
            self.console.print_error(
                f"{self.current_adapter.name} adapter failed: {error_msg}"
            )
            logger.error(
                "Primary adapter %s failed and fallback is disabled: %s",
